"""Ported from ghosttrace/tests/test_deep_trace.py -- bounded agentic
loop (engines/deep_trace.py)."""
...
class TestRunDeepTrace:
    ...
    def test_end_turn_immediately_returns_synthesis(self):
        db = MagicMock()
        with patch("engines.deep_trace.anthropic.Anthropic") as mock_cls:
    ...
    def test_tool_call_counted_and_logged(self):
        db = MagicMock()
        with patch("engines.deep_trace.anthropic.Anthropic") as mock_cls:
            with patch("engines.deep_trace._execute_tool") as mock_exec:
    ...
    def test_budget_enforced(self):
        ...
        with patch("engines.deep_trace.anthropic.Anthropic") as mock_cls:
            with patch("engines.deep_trace._execute_tool") as mock_exec:
                ...
                from engines.deep_trace import run_deep_trace
                from configs.ghosttrace import DEEP_TRACE_MAX_TOOL_CALLS
                result = run_deep_trace(**_TRACE_KWARGS, db=db)
...
class TestExecuteTools:
    ...
    def test_search_cached_filings_store_error_captured(self):
        ...
        with patch("engines.ghosttrace_vector_store.get_store") as mock_store:
