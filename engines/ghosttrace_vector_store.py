"""
engines/ghosttrace_vector_store.py — ported verbatim from
ghosttrace/vector_store.py (import paths only). Filing-text chunking,
embedding, and search over ChromaDB.

ChromaDB runs embedded and in-memory; the index is rebuilt from the Filing
table at startup (reindex_from_db). Embeddings are computed here and
passed to Chroma explicitly — never Chroma's default embedding function
(cold-start time/RAM reasons, unchanged from the original rationale). The
hashed bag-of-words embedder does lexical matching, not true semantics —
an honest, documented tradeoff.
"""
...
from configs.ghosttrace import CHUNK_CHARS, CHUNK_OVERLAP, EMBED_DIM, SEARCH_RESULTS_K

_COLLECTION_NAME = "ghosttrace_filings"
...
def reindex_from_db(db) -> int:
    ...
    the index is in-memory, the filings are in SQLite, and both start empty
    except for what seed data provides."""
    from models.ghosttrace import Filing
