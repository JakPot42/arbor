"""engines/ghosttrace_seed_data.py — ported from ghosttrace/seed_data.py.
...
**One addition from the original:** resolves the focal company against the
shared Company table via resolve_or_create_company() before storing the
Trace.
"""
...
def load_seed_data(db: Session) -> dict:
    ...
    resolution = resolve_or_create_company(db, "Harborview Capital Partners LP")
    ...
    # Guarded anyway: a graph failure must never block seeding.
    try:
